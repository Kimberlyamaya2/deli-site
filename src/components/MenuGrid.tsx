  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-3 md:px-6">
              ...
              return (
                ...
                >
                  ...
                  <div className="mb-4 flex items-center gap-3">
                    <span className="text-[#c9993f] text-xs">✦</span>
                    <h2
                      id={`${id}-heading`}
                      className={`h-display ${tone.heading} text-2xl uppercase tracking-[0.04em] font-bold`}
                    >
                      {section.category}
                    </h2>
                    <span className="flex-1 h-px bg-gradient-to-r from-[#c9993f]/40 to-transparent" />
                  </div>
                  ...
                  >
                    {section.items.map((item) => (
                      <li key={item.name} className="p-4 md:p-5 relative overflow-hidden">
                        {/* Spice accent bar */}
                        <div
                          className={`absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b ${tone.bar}`}
                        />
                        <div className="pl-2 grid grid-cols-[1fr_auto] items-start gap-3">
                          <p className="font-medium text-[#2b211b] text-base md:text-[1.05rem] leading-tight">
                            {item.name}
                            <Badges tags={item.tags} />
                          </p>
                          <span className="font-semibold text-[#8a3a0f] whitespace-nowrap">
                            {priceText(item)}
                          </span>
                        </div>

                        {item.desc && (
                          <p className="mt-1.5 pl-2 text-sm text-[#5a4333] leading-relaxed">
                            {item.desc}
                          </p>
                        )}

                        {item.options && (
                          <details className="group mt-3 ml-2 rounded-lg border border-[#b99d84] bg-[#eadbca] overflow-hidden">
                            <summary
                              className={[
                                "flex items-center justify-between gap-3 cursor-pointer select-none",
                                "px-3 py-2 text-sm text-[#5a4333] hover:text-[#2b211b]",
                                "focus:outline-none focus-visible:ring-2 focus-visible:ring-[#c9993f]/30",
                              ].join(" ")}
                            >
                              <span className="inline-flex items-center gap-2">
                                <svg aria-hidden viewBox="0 0 24 24" className="h-4 w-4 opacity-70" fill="none" stroke="currentColor" strokeWidth="1.6">
                                  <path d="M12 3v3M6 6l2 2M18 6l-2 2M4 12h3M17 12h3M6 18l2-2M18 18l-2-2M12 18v3" />
                                </svg>
                                Customize
                              </span>
                              <svg aria-hidden viewBox="0 0 24 24" className="h-4 w-4 transition-transform duration-200 group-open:rotate-180" fill="none" stroke="currentColor" strokeWidth="1.6">
                                <path d="M6 9l6 6 6-6" />
                              </svg>
                            </summary>
                            ...
                            <div className="px-3 pb-3 pt-2 grid gap-3 sm:grid-cols-2">
                              {Object.entries(item.options).map(([group, values]) => (
                                <div key={group} className="rounded-md border border-[#bda186] bg-[#f3e7db] p-3">
                                  <p className="mb-2 text-[11px] uppercase tracking-wide text-[#6f4d34]">{group}</p>
                                  <div className="flex flex-wrap gap-1.5">
                                    {values.map((v) => (
                                      <span key={v} className="inline-flex items-center rounded-full border border-[#a88463] bg-[#eadbca] px-2.5 py-0.5 text-[11px] text-[#5a4333]">
                                        {v}
                                      </span>
                                    ))}
                                  </div>
      ...
      <div className="mt-10 text-center space-y-2">
        ...
        <p className="text-[11px] text-[#7a563b]">
